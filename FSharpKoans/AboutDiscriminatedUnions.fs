...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BA (_,_) |BA (_,_) -> "A thinker, eh?"
        randomOpinion (BSc(Linguistics, ComputerScience)) |> should equal "Good choice!"
        randomOpinion (BSc (Philosophy, Linguistics)) |> should equal "!!SCIENCE!!"
    ...
    let ``03 We can create a discriminated union using named fields`` () =
        let someDegree = BSc (ComputerScience, Mathematics)          
    ...
    let ``04 Pattern-matching using named fields`` () =
        let result =
            ...
            | __-> "correct" // <-- USE a pattern-match with named fields!
            | _ -> "nope"
