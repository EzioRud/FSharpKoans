...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        ...
        let y = "geralt" // replace with a string
        let z = 7 // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 14 // replace with an integer
        let _ = "Me2" // replace with a string
        let _ = 5 // replace with anything else!
        ()
